...
// by using it we can change the name of (login) function:
...
// through which we will basically ADD the (token-value) of the (user-auth) in our (localStorage):
// 2 = we also have import the (unique-key) from the (same) folder:
// through which we will basically add the (token-value) in the localstorage:
...
export const useAuth = () => {
  // V.IMP= 3  => so for giving the access of the (user-auth) state.
  ...
  // so we have to (return) that (Auth-context):form this custom-hook function:
  ...
  // we can simple get that state with the help of this (custom-hook) function:
...
export const useProvideAuth = () => {
  ...
  // that (state) basically handle or manage by the in build  (useState) hook:
